...
void USlot_cpp::Refresh()
{
	switch (Type)
	{
		case ESlotType::SLOT_Item:
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("PlayerCurrentItemNum = %d"), Player->GetCurrentItemNumber());
			...
			UE_LOG(LogTemp, Warning, TEXT("ItemCount = %d"), data.ItemCount);
			...
			else
			{
				...
				UE_LOG(LogTemp, Warning, TEXT("Have Text = %s"), *(Have->GetText().ToString()));
