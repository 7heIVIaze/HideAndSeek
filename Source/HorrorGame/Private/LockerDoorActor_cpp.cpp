...
ALockerDoorActor_cpp::ALockerDoorActor_cpp()
{
	...
	LockerSound = CreateDefaultSubobject<UAudioComponent>(TEXT("OpenCloseSound"));
	LockerSound->SetupAttachment(LockerDoorMesh);
	static ConstructorHelpers::FObjectFinder<USoundCue>LockerSoundCue(TEXT("/Game/Assets/Sounds/SoundCues/Lockers"));
	if (LockerSoundCue.Succeeded())
	{
		LockerSound->SetSound(LockerSoundCue.Object);
	}
