...
AReaper_cpp::AReaper_cpp()
{
	...
	static ConstructorHelpers::FObjectFinder<USoundCue>DetectedCue(TEXT("/Game/Assets/Sounds/DetecedCue"));
	if (DetectedCue.Succeeded())
	{
		DetectedSound = DetectedCue.Object;
	}

	static ConstructorHelpers::FObjectFinder<USoundCue>PatrolCue(TEXT("/Game/Assets/Sounds/GhostSound"));
...
void AReaper_cpp::Tick(float DeltaTime)
{
	...
	if (bIsPlayerWatch)
	{
		...
		if (CastingTime >= 2 && !bIsCooldown)
		{
			if (GEngine)
				GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, FString::Printf(TEXT("Player Stun")));
			Player->SetPlayerStatus(Player_Status::Stunned);
			bIsCooldown = true;
			CastingTime = 0.f;
		}
	}
	...
	Cast<ACreatureAI>(GetController())->SetControlRotation(GetActorRotation());
	/*ACreatureAI* Controller = Cast<ACreatureAI>(GetController());
