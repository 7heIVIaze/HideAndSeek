...
void ASoulLantern::Tick(float DeltaTime)
{
	...
	if (bIsLightOn)
	{
		...
		if (Timer >= 1.0f)
		{
			...
			//if (GameInstance)
			//{
			//	float HPLossRate = GameInstance->GetHPLossRate();
			//
...
//void ASoulLantern::OnInteract(class AHorrorGameCharacter* Player)
//{
...
//				Player->SetArchiveGetText(NSLOCTEXT("ASoulLantern", "Get_SoulLantern", "Soul Lantern\nis added in archive"));
//				SaveData->SaveData();
//			}
//		}
//
...
//void ASoulLantern::UseInteract(class AHorrorGameCharacter* Player)
//{
...
//void ASoulLantern::HideInteract(class AHorrorGameCharacter* Player)
//{
//	// 빛이 나는 지를 현재 값을 반전시켜 저장하고, 플레이어에게도 저장함.
//	bIsLightOn = !bIsLightOn;
//	UE_LOG(LogTemp, Warning, TEXT("Player: LanternOn : %s"), bIsLightOn ? TEXT("true") : TEXT("false"));
