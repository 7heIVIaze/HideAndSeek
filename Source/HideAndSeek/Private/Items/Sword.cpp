...
void ASword::OnSwordBeginOverlap(class UPrimitiveComponent* OverlappedComp, class AActor* OtherActor, class UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	// 충돌체가 있을 경우
	if (OtherComp != nullptr && OtherActor != this && OtherActor != nullptr && OtherComp->IsA<USkeletalMeshComponent>())
	{
		// 그 액터가 요괴인 경우
		if (ACreatureClass* Yokai = Cast<ACreatureClass>(OtherActor))
		{
			// 처리 효과음을 재생함.
			if (ExorcismSound)
			{
...
// 검의 충돌체를 설정하는 함수.
void ASword::SetOnCollision(const bool value)
{
	if (value) // 만약 true(즉 공격 모션이 시작된 경우)
	{
		CapsuleComp->OnComponentBeginOverlap.AddDynamic(this, &ASword::OnSwordBeginOverlap);
	}
	else // 만약 false(즉 공격 모션이 끝나거나 크리쳐를 처치했거나 아닌 경우)
	{
		CapsuleComp->OnComponentBeginOverlap.RemoveDynamic(this, &ASword::OnSwordBeginOverlap);
	}
}

// 플레이어가 칼을 휘두르는 모션을 할 때 호출되는 함수.
void ASword::SetShouldExorcism(const bool value)
{
