...
void AHorrorGameCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		// Quit Game 
		//EnhancedInputComponent->BindAction(GamePauseAction, ETriggerEvent::Started, this, &AHorrorGameCharacter::Pause);
...
//		Inventory[InventoryNum].Type = CigarLightData->Type;
...
//			Inventory[InventoryNum].Type = FlashLightData->Type;
//			Inventory[InventoryNum].ItemCount = 1;
//
//			// 만약 아이템이 없었다면 0으로 인덱스를 설정함
//			if (CurrentItemNum < 0)
...
//			Inventory[InventoryNum].Type = KeyData->Type;
//			Inventory[InventoryNum].ItemCount += 1;
//
//			// 만약 아이템이 없었다면 0으로 인덱스를 설정함
//			if (CurrentItemNum < 0)
...
//			Inventory[InventoryNum].Type = TimerData->Type;
//			Inventory[InventoryNum].ItemCount += 1;
//
//			// 만약 아이템이 없었다면 0으로 인덱스를 설정함
//			if (CurrentItemNum < 0)
...
//			Inventory[InventoryNum].Type = SwordData->Type;
//			SwordCount = 1;
...
//			Inventory[InventoryNum].Type = BellData->Type;
//			BellCount = 1;
...
//			Inventory[InventoryNum].Type = MirrorData->Type;
//			MirrorCount = 1;
...
//			Inventory[InventoryNum].Type = ExtinguisherData->Type;
//			Inventory[InventoryNum].ItemCount = 1;
//
//			// 만약 아이템이 없었다면 0으로 인덱스를 설정함
//			if (CurrentItemNum < 0)
...
//			Inventory[InventoryNum].Type = CutterData->Type;
//			Inventory[InventoryNum].ItemCount = 1;
//
//			// 만약 아이템이 없었다면 0으로 인덱스를 설정함
//			if (CurrentItemNum < 0)
...
//			Inventory[InventoryNum].Type = LanternData->Type;
//			Inventory[InventoryNum].ItemCount = 1;
//
//			// 만약 아이템이 없었다면 0으로 인덱스를 설정함
//			if (CurrentItemNum < 0)
...
//			Inventory[InventoryNum].Type = GlowStickData->Type;
//			Inventory[InventoryNum].ItemCount += 1;
//
//			// 만약 아이템이 없었다면 0으로 인덱스를 설정함
//			if (CurrentItemNum < 0)
...
//	if (Lantern)
//	{
//		if (ALightItem* SoulLantern = Cast<ALightItem>(Lantern->GetChildActor()))
//		{
//			SoulLantern->UseInteract(this);
//		}
//	}
...
bool AHorrorGameCharacter::UseGlowstick()
{
	...
	if (GlowStickClass)
	{
		...
		//AActiveItem* GlowStick = GetWorld()->SpawnActor<AActiveItem>(GlowStickClass, SpawnLocation, FRotator(0.f, 0.f, 0.f), SpawnParams);
...
//	if (SelectedItemNumber == 2) // FlashLight
//	{
//		if (GameUIWidget)
//		{
...
//	if (SelectedItemNumber == 8) // Extinguisher
//	{
//		if (GameUIWidget)
//		{
...
//	if (SelectedItemNumber == 9) // Cutter
//	{
//		if (GameUIWidget)
//		{
...
//	if (SelectedItemNumber == 10) // SoulLantern
//	{
...
//	if (SelectedItemNumber == 7) // Mirror
//	{
...
//		return 0;
//	}
//	SelectedItemNumber = Inventory[CurrentItemNum].ItemNumber;
...
void AHorrorGameCharacter::SwitchItem(const FHorrorGameItemData& CurrentItemData, int32 CurrentItemIndex)
{
	switch (CurrentItemData.ItemType)
	{
		...
		case EItemType::ITEM_Consumable:
		{
			...
			//MirrorComp->SetHiddenInGame(true);
			if (Mirror)
			{
				Mirror->SetItemVisibility(true);
			}

			//FEComp->SetHiddenInGame(true);
			if (FireExtinguisher)
			{
			...
			switch (CurrentItemData.ItemNumber)
			{
				...
				{
					//SwordComp->SetHiddenInGame(false);
				...
				{
					//MirrorComp->SetHiddenInGame(false);
					if (Mirror)
					{
...
//		{
//			if (auto Yokai = Cast<ACreatureClass>(OverlapResult.GetActor()))
//			{
//				/*if (GEngine)
...
// 인벤토리의 아이템들을 아이템 번호 순으로 정렬하는 함수.
//void AHorrorGameCharacter::InventorySorting()
//{
//	if (InventoryNum < 0)
//	{
//		SelectedItemNumber = 12;
...
//
//	// 선택된 아이템의 인덱스를 찾기 위한 로직
//	for (int i = 0; i < InventoryNum; ++i)
//	{
//		if (Inventory[i].ItemNumber == SelectedItemNumber)
//		{
//			CurrentItemNum = i;
//		}
//	}
...
void AHorrorGameCharacter::HandleFlashlightSway(float Value)
{
	...
	//UE_LOG(LogTemp, Warning, TEXT("%f, %f, %f"), SpringArmComp->GetComponentRotation().Pitch, SpringArmComp->GetComponentRotation().Yaw, SpringArmComp->GetComponentRotation().Roll);
}