...
const FName AYokaiAIController::PatrolTargetKey(TEXT("PatrolTarget"));
const FName AYokaiAIController::NoiseTargetKey(TEXT("NoiseTargetKey"));
const FName AYokaiAIController::HomePosKey(TEXT("HomePos"));
const FName AYokaiAIController::PatrolPosKey(TEXT("PatrolPos"));
const FName AYokaiAIController::TargetKey(TEXT("Target"));
const FName AYokaiAIController::TargetLocation(TEXT("TargetLocation"));
const FName AYokaiAIController::CanSeePlayer(TEXT("CanSeePlayer"));
const FName AYokaiAIController::NoiseDetected(TEXT("NoiseDetected"));
const FName AYokaiAIController::Stunned(TEXT("Stunned"));
const FName AYokaiAIController::LockerLighting(TEXT("LockerLighting"));
const FName AYokaiAIController::LockerTargetKey(TEXT("LockerTarget"));

// Constructor
AYokaiAIController::AYokaiAIController()
{
	// AI Controller는 Tick 비활성화할 것임. Actor만 해도 충분함.
	PrimaryActorTick.bCanEverTick = false;

	// 블랙보드와 비헤이비어트리 설정
	SetupBlackboardAndBehaviorTree();

	// AI Perception 설정
	SetPerception();
}

void AYokaiAIController::BeginPlay()
{
	Super::BeginPlay();
...
void AYokaiAIController::OnPossess(APawn* InPawn)
{
...
void AYokaiAIController::StopAI()
{
...
//UBlackboardComponent* AYokaiAIController::GetBlackboard() const
//{
//	return BlackboardComp;
//}

// 플레이어가 AI Perception의 주 감각에 의해 감지될 경우 처리할 함수
void AYokaiAIController::OnTargetDetected(AActor* Actor, FAIStimulus const Stimulus)
{
	...
	if (Stimulus.WasSuccessfullySensed())
	{
		...
		if (auto const player = Cast<AHorrorGameCharacter>(Actor))
		{
			...
			FVector YokaiLocation = GetPawn()->GetActorLocation();
			PlayerLocation.Z = YokaiLocation.Z;
			float Dist = FMath::Abs(FVector::Distance(YokaiLocation, PlayerLocation));
			...
			else
			{
				...
				{
					if (Dist <= NoLightSightRadius) // 2000 이하 거리 이내에 있으면 어느 상황에서든 감지
					{
						GetBlackboardComponent()->SetValueAsObject(TargetKey, player);
						GetBlackboardComponent()->SetValueAsVector(TargetLocation, player->GetActorLocation());
					}
...
void AYokaiAIController::SetPerception()
{
	...
	GetPerceptionComponent()->OnTargetPerceptionUpdated.AddDynamic(this, &AYokaiAIController::OnTargetDetected);
	GetPerceptionComponent()->ConfigureSense(*SightConfig);
}

void AYokaiAIController::SetStunned(bool value)
{
	/*if (value)
	{
		bIsStunned = value;
	}*/
	GetBlackboardComponent()->SetValueAsBool(Stunned, value);
}

void AYokaiAIController::EndChase()
{
...
void AYokaiAIController::SetupBlackboardAndBehaviorTree()
{
