...
ACreatureClass::ACreatureClass()
{
	...
	BoxCollision->OnComponentBeginOverlap.AddDynamic(this, &ACreatureClass::SoundBeginOverlap);
	BoxCollision->OnComponentEndOverlap.AddDynamic(this, &ACreatureClass::SoundEndOverlap);
...
void ACreatureClass::BeginPlay()
{
	...
	if (DissolveCurveFloat)
	{
		...
		TimelineProgress.BindDynamic(this, &ACreatureClass::ChangeMaterialInstance);
		DissolveTimeline.AddInterpFloat(DissolveCurveFloat, TimelineProgress);

		TimelineFinish.BindDynamic(this, &ACreatureClass::DissolveFinish);
...
void ACreatureClass::StartChase()
{
	...
	if (!bIsChase)
	{
		...
		if (PlayerCharacter)
		{
			PlayerCharacter->SetPlayerStatus(EPlayerStatus::Chased);
		}
	}
}
...
void ACreatureClass::EndChase()
{
	...
	if (bIsChase)
	{
		...
		if (PlayerCharacter)
		{
			PlayerCharacter->SetPlayerStatus(EPlayerStatus::Survive);
		}
	}
}

// 플레이어를 잡았는지 설정하는 함수
void ACreatureClass::SetIsCatch(bool Value)
{
	bIsCatch = Value;
}

// 공격 애니메이션이 끝났는지 설정하는 함수
void ACreatureClass::SetAnimFinish(bool Value)
{
	// 애니메이션이 끝났음을 설정
	bAnimFinish = Value;
	UWorld* World = GetWorld();
	...
	else
	{
		...
		{
			PlayerCharacter->SetPlayerStatus(EPlayerStatus::Died);
		}
	}
}
...
void ACreatureClass::SoundBeginOverlap(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (OtherActor != nullptr && OtherActor != this && OtherComp != nullptr)
	{
		...
		if (auto PlayerCharacter = Cast<AHorrorGameCharacter>(OtherActor))
		{
			...
			{
				PlayerCharacter->AddConfusionPoint(1);
			}
	...
	if (OtherActor != nullptr && OtherActor != this && OtherComp != nullptr)
	{
		...
		if (auto PlayerCharacter = Cast<AHorrorGameCharacter>(OtherActor))
		{
			...
			if (PlayerCharacter->CreatureNum <= 0)
			{
				...
				Cast<ALightItem>(PlayerCharacter->CigarlighterComp->GetChildActor())->LightComponent->SetIntensity(PlayerCharacter->CigarIntensity);
				Cast<ALightItem>(PlayerCharacter->FlashlightComp->GetChildActor())->LightComponent->SetIntensity(PlayerCharacter->FlashIntensity);
		...
		if (auto HideObject = Cast<AHideObject>(OtherActor))
		{
			...
			if (HideObject->CreatureNum <= 0)
			{
				...
				Cast<ALightItem>(HideObject->CigarlighterComp->GetChildActor())->LightComponent->SetIntensity(HideObject->Intensity);
				Cast<ALightItem>(HideObject->CigarlighterComp->GetChildActor())->LightComponent->SetIntensity(HideObject->Intensity);
...
void ACreatureClass::DetectPlayerHidingObject(class AHideObject* DetectedObject)
{
...
// 순찰 성공 여부를 설정하는 함수
void ACreatureClass::SetPatrolSuccess(bool value)
{
...
bool ACreatureClass::GetIsDied()
{
	return bIsDied;
}

bool ACreatureClass::GetIsCatch()
{
	return bIsCatch;
}

bool ACreatureClass::GetAnimFinish()
{
	return bAnimFinish;
}

// Sphere 충돌체에 플레이어가 충돌한 경우 호출할 콜백 함수.
void ACreatureClass::CatchBeginOverlap(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherIndexBody, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (!bIsStunned && !bIsDied && !bIsTimeStop)
	{
		...
		if (OtherActor != this && OtherActor != nullptr && OtherComp != nullptr)
		{
			...
			if (auto PlayerCharacter = Cast<AHorrorGameCharacter>(OtherActor))
			{
				...
				if ((PlayerCharacter->GetPlayerStatus() == EPlayerStatus::Survive || PlayerCharacter->GetPlayerStatus() == EPlayerStatus::Chased) && PlayerCharacter->GetPlayerStatus() != EPlayerStatus::Hiding)
				{
...
bool ACreatureClass::GetPatrolSuccess()
{
	return bIsPatrolSuccess;
}

void ACreatureClass::SetCreatureCollision(bool value)
{
...
void ACreatureClass::SetIsStop(bool inIsStop)
{
	bIsStop = inIsStop;
}

bool ACreatureClass::GetIsStop()
{
	return bIsStop;
}
...
void ACreatureClass::OpenDoor()
{
	...
	if (bIsHit)
	{
		if (HitResult.GetActor())
		{
			...
			if (ADoorClass* Door = Cast<ADoorClass>(HitActor))
			{
